...
def plot_robot(q_val):
    ...
    ax.plot(*zip(P3, P1, P5), "b-o", lw=4, ms=10, zorder=3, label="Stance leg")
    # ax.plot(*zip(P5, Pt), "k-o", lw=5, ms=10, zorder=3, label="Torso")
    ax.plot(*zip(P5, P2, P4), "r-o", lw=4, ms=10, zorder=3, label="Free leg")
    ax.axhline(0, color="gray", lw=1.5, linestyle="--")

    # Joint labels with individual offsets to avoid overlap
    joint_labels = [
        ("$P_3$ (stance foot)", P3, (-15, -20)),
        ("$P_{k1}$ (stance knee)", P1, (-120, -5)),
        ("$P_h$ (hip)", P5, (-80, 10)),
        # ("$P_t$ (torso top)", Pt, (12, 5)),
        ("$P_{k2}$ (free knee)", P2, (12, -5)),
    ...
    ax.set_aspect("equal")
    margin = 0.3
    all_x = [p[0] for p in [P3, P1, P5, P2, P4]]
    all_y = [p[1] for p in [P3, P1, P5, P2, P4]]
...
def plot_frozen_body_snapshots(sol1, sol2, n_frames=6):
    ...
    P4_at_impact = get_skeleton(q_at_impact)[4]
...
def animate_frozen_and_impact(sol1, sol2, interval=30):
    ...
    q_at_impact = sol1.y[:5, -1]
    P4_at_impact = get_skeleton(q_at_impact)[4]  # free foot position
    x_off = P4_at_impact[0]
